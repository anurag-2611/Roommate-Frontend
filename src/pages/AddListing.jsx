...
export const AddListing = () => {
  ...
  // Once a room has been successfully added, this locks the form so the
  // user can only add ONE room per visit to this page. They must click
  // "Add Another Room" (which fully resets state) to submit again.
  ...
  const handleSubmit = async (e) => {
    ...
    // Guard against double submit AND against submitting a second room
    // while one is already successfully added in this session.
    ...
    try {
      ...
      if (Array.isArray(formData.videos) && formData.videos.length > 0) {
      ...
      // Axios only resolves (lands here) for 2xx responses by default,
      // and rejects (goes to catch) for 4xx/5xx. So if we're here, the
      // request succeeded — trust that instead of a possibly-missing
    ...
    <div className="w-full min-h-screen flex flex-col items-center justify-center bg-linear-to-br from-blue-200 via-indigo-100 to-blue-50 py-6 sm:py-10 px-3 sm:px-4 text-blue-950 font-medium">
      ...
      <div className="w-full mt-20 max-w-5xl">
        ...
        <div className="bg-white/70 w-full shadow-2xl rounded-2xl p-4 sm:p-8 md:p-12 transition-all duration-500 animate-fade-in-step">
          {submitted ? (
            <div className="flex flex-col items-center justify-center text-center py-10 sm:py-16 gap-4">
              ...
              <p className="text-blue-700 max-w-md">
                Your listing has been submitted. You can add another room
                whenever you're ready.
              </p>
              <button
                type="button"
                onClick={handleAddAnotherRoom}
                className="mt-4 bg-linear-to-r from-blue-500 to-indigo-500 text-white font-bold px-6 py-2 rounded-lg shadow-lg hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 hover:shadow-xl text-sm sm:text-base"
              >
                Add Another Room
              </button>
            </div>
